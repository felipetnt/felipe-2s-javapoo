package View;

import Model.*;

import java.util.ArrayList;
import java.util.List;
import java.util.Scanner;

public class Menu {
    public static void showMenu(){

        ArrayList<Pessoa> pessoas = new ArrayList<>();
        ArrayList<Material> materiais = new ArrayList<>();
        ArrayList<Emprestimo> emprestimos = new ArrayList<>();

        int opcao;

        do {
            System.out.println("===== SISTEMA DE BIBLIOTECA =====");
            System.out.println("1 - Cadastrar Usuário");
            System.out.println("2 - Cadastrar Funcionário");
            System.out.println("3 - Listar Pessoas");
            System.out.println("4 - Cadastrar Livro");
            System.out.println("5 - Cadastrar Revista");
            System.out.println("6 - Listar Materiais");
            System.out.println("7 - Cadastrar Empréstimo");
            System.out.println("8 - Listar Empréstimos");
            System.out.println("0 - Sair");
            System.out.print("Escolha uma opção: ");
            opcao = new Scanner(System.in).nextInt();

            switch (opcao) {
                case 1:
                    System.out.println("Voce escolheu inserir");
                    System.out.print("Nome: ");
                    String userName = new Scanner(System.in).nextLine();
                    System.out.print("Email: ");
                    String userEmail = new Scanner(System.in).nextLine();
                    System.out.print("Matrícula: ");
                    int userMatricula = new Scanner(System.in).nextInt();

                    pessoas.add(new Usuario(userName, userEmail, userMatricula));
                    System.out.println("Usuário cadastrado com sucesso!\n");
                    break;

                case 2:
                    System.out.print("Nome: ");
                    String nomeF = new Scanner(System.in).nextLine();
                    System.out.print("Email: ");
                    String emailF = new Scanner(System.in).nextLine();
                    System.out.print("Cargo: ");
                    String cargo = new Scanner(System.in).nextLine();
                    pessoas.add(new Funcionario(nomeF, emailF, cargo));
                    System.out.println("Funcionário cadastrado com sucesso!\n");
                    break;

                case 3:
                    System.out.println("=== Lista de Pessoas ===");
                    for (Pessoa p : pessoas) {
                        p.exibirInfo();
                        System.out.println("-------------------");
                    }
                    break;

                case 4:
                    System.out.print("Título: ");
                    String livroTitle = new Scanner(System.in).nextLine();
                    System.out.print("Ano de publicação: ");
                    int livroAnoPublication = new Scanner(System.in).nextInt();

                    System.out.print("Autor: ");
                    String autor = new Scanner(System.in).nextLine();
                    materiais.add(new Livro(livroTitle, livroAnoPublication, autor));
                    System.out.println("Livro cadastrado!\n");
                    break;

                case 5:
                    System.out.print("Título: ");
                    String RevistaTitle = new Scanner(System.in).nextLine();
                    System.out.print("Ano de publicação: ");
                    int RevistaAnoPublication = new Scanner(System.in).nextInt();
                    System.out.print("Edição: ");
                    int RevistaEdicao = new Scanner(System.in).nextInt();
                    materiais.add(new Revista(RevistaTitle, RevistaAnoPublication, RevistaEdicao));
                    System.out.println("Revista cadastrada!\n");
                    break;

                case 6:
                    System.out.println("=== Lista de Materiais ===");
                    for (Material m : materiais) {
                        m.descricao();
                        System.out.println("-------------------");
                    }
                    break;

                case 7:
                    if (pessoas.isEmpty() || materiais.isEmpty()) {
                        System.out.println("É necessário ter pelo menos uma pessoa e um material cadastrados.\n");
                        break;
                    }
                    System.out.println("Escolha o usuário (índice):");
                    for (int i = 0; i < pessoas.size(); i++) {
                        System.out.println(i + " - " + pessoas.get(i).getNome());
                    }
                    int idxUsuario = new Scanner(System.in).nextInt();

                    System.out.println("Escolha o material (índice):");
                    for (int i = 0; i < materiais.size(); i++) {
                        System.out.println(i + " - " + materiais.get(i).getTitulo());
                    }
                    int idxMaterial = new Scanner(System.in).nextInt();

                    System.out.print("Data de empréstimo: ");
                    String dataE = new Scanner(System.in).nextLine();
                    System.out.print("Data de devolução: ");
                    String dataD = new Scanner(System.in).nextLine();

                    emprestimos.add(new Emprestimo(pessoas.get(idxUsuario), materiais.get(idxMaterial), dataE, dataD));
                    System.out.println("Empréstimo cadastrado!\n");
                    break;

                case 8:
                    System.out.println("=== Lista de Empréstimos ===");
                    for (Emprestimo e : emprestimos) {
                        e.exibirDetalhes();
                        System.out.println("-------------------");
                    }
                    break;

                case 0:
                    System.out.println("Encerrando o sistema...");
                    break;

                default:
                    System.out.println("Opção inválida!\n");
            }

        } while (opcao != 0);
    }
}
